...
st.markdown(
    ...
    .nav-button { background-color: transparent; border: none; font-weight:600;
                  color: #3b2f1f; padding: 8px 14px; border-radius:6px; }
    ...
    .score-card { padding:20px; border-radius:12px; color:white;
                  box-shadow: 0 6px 20px rgba(13,27,42,0.12); margin-top:12px;
                  border:1px solid rgba(255,255,255,0.06); max-width:720px; }
...
# -----------------------------------------------------------
    ...
    terme = st.text_input(
        "Rechercher (ex: Lactobacillus plantarum OR phenyllactic acid OR plantaricin)"
    )
    ...
    if terme:
        ...
        try:
            ...
            if ids:
                ...
                for pid in ids:
                    ...
                    line = (
                        f"- <b><a href='{url}' target='_blank' "
                        f"style='color:#0A84FF;text-decoration:none'>{html.escape(title)}</a></b>"
                    )
                    extra = f" — {html.escape(source)} {html.escape(pubdate)}" if source else ""
                    st.markdown(line + extra, unsafe_allow_html=True)
            else:
            ...
            if hits:
                for entry in hits:
                    ...
                    name = entry.get("proteinDescription", {}).get(
                        "recommendedName", {}
                    ).get("fullName", {}).get("value", "") or acc

                    url = f"https://rest.uniprot.org/uniprotkb/{acc}"

                    st.markdown(
                        f"- <a href='{url}' target='_blank' "
                        f"style='color:#0A84FF;text-decoration:none'><b>{html.escape(name)}</b></a>"
                        f" — Accession : `{acc}`",
                        unsafe_allow_html=True,
                    )
            else:
        ...
        try:
            encoded = urllib.parse.quote_plus(terme)
            link = f"https://www.rcsb.org/search?request={{\"query\":\"{encoded}\"}}"

            st.markdown(
                f"- <a href='{link}' target='_blank' style='color:#0A84FF;'>Ouvrir RCSB</a>",
                unsafe_allow_html=True,
            )
        except:
            st.info("Erreur PDB")
